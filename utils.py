...
def interpolate(origin_y, valid, new_valid, num_to_interpolate = 1):
...
#MODIFY 원래는 2fps를 10fps로 interpolation하기 위해서 num_to_interpolate가 5였다. ->1 (우리는 1FPS로 하였음)
def get_tracks_from_frames(timestamp_ns, nuscenes: NuScenes, scene_info, frames, num_to_interpolate=5,):
    timestamp_ns = timestamp_ns
    episode_len = len(frames)
    # Fill tracks
    all_objs = set()
    for frame in frames: #frames 즉, 1scene을 이루는 frame뭉치안에서 frame하나별로 보겠다
    ...
    tracks = {
        k: dict(
            ...
            #track_length를 interpolate하기전에는 일단 이대로 들고옴
            metadata=dict(track_length=episode_len, type=MetaDriveType.UNSET, object_id=k, original_id=k, timestamp_ns = timestamp_ns)
        )
...
#실질적으로 frame의 모든 정보를 객체별로 하나로 묶는 것은 여기서부터 진행
    for frame_idx in range(episode_len):
        # Record all agents' states (position, velocity, ...)
        for id, state in frames[frame_idx].items(): #한객체별로 담기
            # Fill type
            ...
            #MODIFY
            tracks[id]["metadata"]["timestamp_ns"] = timestamp_ns
    ...
    # interpolate
    #MJTODO: 보간 되는 시간만큼 timestamp를 늘려야함
    interpolate_tracks = {}
    for id, track, in tracks.items():
        interpolate_tracks[id] = copy.deepcopy(track)
        interpolate_tracks[id]["metadata"]["track_length"] = episode_len
        ...
        #MODIFY
        #개 짜치는 코딩 ㅋㅋ new_timestamp_ns = [item for item in timestamp_ns for _ in range(num_to_interpolate)]
        #array_timestamp_ns = np.array(track["metadata"]["timestamp_ns"]).reshape(len(track["metadata"]["timestamp_ns"]),1)
        #new_timestamp_ns = interpolate(array_timestamp_ns, track["state"]["valid"], new_valid)
        #interpolate_tracks[id]["metadata"]["timestamp_ns"] = new_timestamp_ns """     
    return interpolate_tracks

def get_tracks_from_1FPS(timestamp_ns, nuscenes: NuScenes, scene_info, frames, num_to_interpolate=1):
    timestamp_ns = timestamp_ns
    episode_len = len(frames)
    # Fill tracks
    all_objs = set()
...
#실질적으로 frame의 모든 정보를 객체별로 하나로 묶는 것은 여기서부터 진행
    ...
    for id, track, in tracks.items():
        ...
        # position
        interpolate_tracks[id]["state"]["position"] = track["state"]["position"]


        # velocity
        interpolate_tracks[id]["state"]["velocity"] = track["state"]["velocity"]
        vel = interpolate_tracks[id]["state"]["position"][1:] - interpolate_tracks[id]["state"]["position"][:-1]
        interpolate_tracks[id]["state"]["velocity"][:-1] = vel[..., :2] / 0.5
        interpolate_tracks[id]["state"]["velocity"][-1] = interpolate_tracks[id]["state"]["velocity"][-2]
        ...
        array_timestamp_ns = np.array(track["metadata"]["timestamp_ns"]).reshape(len(track["metadata"]["timestamp_ns"]),1)
        #new_timestamp_ns = interpolate(array_timestamp_ns, track["state"]["valid"], new_valid)
        interpolate_tracks[id]["metadata"]["timestamp_ns"] = array_timestamp_ns     
    return interpolate_tracks

#MJTODO: get_map_features확인해서 map어떻게 쓰는지 확인
...
def convert_nuscenes_scenario(
    ...
    scenario_log_interval = 0.5
    frames, scene_info, timestamp_ns, extracted_sample_token = frames_scene_info
    result = SD()
    ...
    #MODIFY #MJTODO: 제대로 된건지 확인해봐야함
    result[SD.METADATA]["scene_token"] = scene_info["token"]
    result[SD.METADATA]["sample_rate"] = scenario_log_interval
    result[SD.METADATA][SD.TIMESTEP] = np.arange(0., (len(frames)*0.5), 0.5)
    #MODIFY
    result[SD.METADATA][SD.TIMESTAMP_NS] = timestamp_ns
    result[SD.METADATA]["sample_token"] = extracted_sample_token
    # interpolating to 0.1s interval
    #MODIFY
    result[SD.TRACKS] = get_tracks_from_1FPS(timestamp_ns ,nuscenes, scene_info, frames, num_to_interpolate=5,)
...
#MODIFY: 각 frame에 맞는 timestamp값을 가지고 오게끔 코드 수정했음
def extract_frames_scene_info(scene, nuscenes):
    ...
    while current_frame["token"] != scene_info["last_sample_token"]:
        ...
        #MODIFY
        sample_token.append(current_frame["token"])
        timestamp_ns.append(current_frame["timestamp"])
        current_frame = nuscenes.get("sample", current_frame["next"])
    frames.append(parse_frame(current_frame, nuscenes))
    sample_token.append(current_frame["token"])
    timestamp_ns.append(current_frame["timestamp"])
    
    #INFO: 밑의 assert는 가지고 오는 frame의 갯수 만큼 sample_token과 timestamp_ns도 맞아야함을 알려주는 코드
    assert len(frames) == len(sample_token) and len(frames) == len(timestamp_ns)
    assert current_frame["next"] == ""
    assert len(frames) == scene_info["nbr_samples"], "Number of sample mismatches! "
    assert scene_token == scene_info["token"], "Scene_token mismatches!"
    
    
    return frames, scene_info, timestamp_ns, sample_token
...
#MODIFY    
#MJTODO: 이거 조금 더 깔끔하게 코드 짜보기
    if version == "v1.0-trainval_train":
        nusc_scene_train = [s for s in nusc.scene if s['name'] in train]
        return nusc_scene_train, [nusc for _ in range(num_workers)]
    elif version == "v1.0-trainval_val":
        nusc_scene_val = [v for v in nusc.scene if v["name"] in val]
        return nusc_scene_val, [nusc for _ in range(num_workers)]
